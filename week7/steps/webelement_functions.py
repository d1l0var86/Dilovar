...
def test_checkbox_is_displayed():
    # Scenario: is_displayed(), is_selected(), is_enabled()
    launch_website("https://letskodeit.teachable.com/p/practice")

    xpath1 = "//legend[contains(text(),'Checkbox Example')]"
    checkbox_example = driver.find_element_by_xpath(xpath1)
    print(f"Checkbox Example is displayed: {checkbox_example.is_displayed()}")

    bmw_option = None
    if checkbox_example.is_displayed():
        bmw_option = driver.find_element_by_xpath("//input[@id='bmwcheck']")
        bmw_option.click()
        time.sleep(3)
    else:
        print("Checkbox example is not displayed")

    print("bmw option is clicked")
    print(f" Is bmw option is selected: {bmw_option.is_selected()}")
    assert bmw_option.is_selected(), "bmw Option is not selected"
    print("Test is successfully passed!!!")
    time.sleep(3)
    driver.close()
